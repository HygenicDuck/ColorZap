...
public class JSON
{
			...
			if (c == '"') {
				...
				if (c == '"') {
					...
					if (remainingLength >= 4) {
						...
						//surrogate range check for EMOJIs
						...
						if( highSurrogate >= lowerRange && highSurrogate <= upperRange )
						{
							...
							string converted = Char.ConvertFromUtf32 ((int)codePointTest);
	...
	protected static object ParseNumber (char[] json, ref int index, ref bool success)
	{
		...
		{
			//	the check here is for 10 AND 11 chars as we need to account for the minus character for negative values
			long longNumber;
			success = long.TryParse(token, out longNumber);
